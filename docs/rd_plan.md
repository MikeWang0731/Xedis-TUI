...
- [x] 实现连接管理器：支持单机、Sentinel、Cluster、TLS 加密连接与密码认证。
- [x] 搭建 4 种分栏预设（平衡 58%、沉浸 75%、监控 35%、纯流 100%）与 `F5` 快捷切换。
- [x] 实现单机模式下的原生 Redis 命令执行与 Raw 输出。
...
- [x] 实现 Safety Guard 高危命令拦截阻断弹窗与新人新手排障知识库（`F1` 帮助）。
- [x] 优化大数据量查询下的终端滚动流畅度与内存占用（虚拟滚动列表）。
- [x] 编写全面的单元测试与 Redis 集群 Mock 测试。
- [ ] CI/CD 跨平台构建发布（Homebrew、Cargo install、Github Releases 二进制 - 暂缓）。
